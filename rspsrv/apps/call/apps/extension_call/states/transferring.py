...
from rspsrv.apps.call.call_control.channel import ChannelIntent, ChannelContext


class TransferringState(SimpleState):
    state_name = ExtensionCallStateName.TRANSFERRING
    ...
    def on_transferee_destroyed(self, raw_channel, event):
        ...
        self.cleanup()
        if self.owner == ExtensionEvent.Caller:
            cause = EndingCause.CALLEE_ATRANSFERRED
        else:
            cause = EndingCause.CALLER_ATRANSFERRED
        self.machine.change_state(self.owner.ATRANSFERRED, cause=cause)
    ...
    def on_transferor_dtmf_received(self, raw_channel, event):
        ...
        if digit == SimpleEvent.DTMF.DTMF_OCTOTHORPE:
            ...
            if self.owner == ExtensionEvent.Caller:
                event = ExtensionEvent.Caller.ATRANSFERRING_CANCELED
                self.machine.callee_channel.intent = ChannelIntent.CALL
                self.machine.channel.context = ChannelContext.INTERNAL
                self.handlers.bridge.add_channel(channel=self.machine.channel)

            else:
                event = ExtensionEvent.Callee.ATRANSFERRING_CANCELED
                self.machine.channel.intent = ChannelIntent.CALL
                self.machine.callee_channel.context = ChannelContext.INTERNAL
        ...
        if digit == SimpleEvent.DTMF.DTMF_STAR:
            self.transferor.move_spy_iccs(self.transferee)
    ...
    def enter(self, target_number, transferor, transferee, owner):
        ...
        self.events.transferee_destroyed = self.transferee.on_event(SimpleEvent.Channel.DESTROYED,
                                                                    self.on_transferee_destroyed)
        self.events.transferee_hungup = self.transferee.on_event(SimpleEvent.Channel.HANGUP_REQUEST,
                                                                 self.on_transferee_hungup)

        self.events.transferor_destroyed = self.transferor.on_event(SimpleEvent.Channel.DESTROYED,
                                                                    self.on_transferor_destroyed)
        self.events.transferor_hungup = self.transferor.on_event(SimpleEvent.Channel.HANGUP_REQUEST,
                                                                 self.on_transferor_hungup)

        self.events.transferor_dtmf_received = self.transferor.on_event(SimpleEvent.Channel.DTMF_RECEIVED,
